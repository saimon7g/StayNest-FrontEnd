...
export async function Step2PUT(data) {
    ...
    try {
        const response = await fetch("http://127.0.0.1:8000/host/api/property_registration/step2/6/", {
            method: "PUT",
            headers: {
                "Content-Type": "application/json"
            },
...
export async function Step2GET() {
    ...
    try {
        const response = await fetch("http://127.0.0.1:8000/host/api/property_registration/step2/3/");
        const result = await response.json();
        console.log(result);
        return result;
    }
